...
def depth(f, b, disp):
    result = np.zeros_like(disp, dtype=np.float64)

    for i in tqdm(range(len(disp)), desc="Depth"):
...
for directory in directories:
    ...
    novel_views = generate_novel_views(imgLcolor, depth_left, K, 0.1)
    ...
    cv2.imwrite(os.path.join(output_dir, 'depth_l.jpg'), depth_left)
    cv2.imwrite(os.path.join(output_dir, 'depth_r.jpg'), depth_right)

    count = 0
    for novel_view in novel_views:
        cv2.imwrite(os.path.join(output_dir, f'synth_{count + 1}.jpg'), novel_view)
        count += 1
